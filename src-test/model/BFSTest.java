...
public class BFSTest {
	...
	public void setup() {
		...
		PathFinder<GridLocation> pathFinder = new BFS<GridLocation>(new Cell[4][4]);
		// Trap Cell[5][0]
		trickyGrid[4][0].addActor(new Robot(blockedCellOne, 1, MockWarehouse.CHARGING_POD, pathFinder));
		trickyGrid[4][1].addActor(new Robot(blockedCellTwo, 1, MockWarehouse.CHARGING_POD, pathFinder));
		trickyGrid[5][1].addActor(new Robot(blockedCellThree, 1, MockWarehouse.CHARGING_POD, pathFinder));

		//Create complex path from [5][2] to [0][4]
		trickyGrid[4][2].addActor(new Robot(blockedCellFour, 1, MockWarehouse.CHARGING_POD, pathFinder));
		trickyGrid[4][3].addActor(new Robot(blockedCellFive, 1, MockWarehouse.CHARGING_POD, pathFinder));
		trickyGrid[4][4].addActor(new Robot(blockedCellSix, 1, MockWarehouse.CHARGING_POD, pathFinder));
		trickyGrid[2][5].addActor(new Robot(blockedCellSeven, 1, MockWarehouse.CHARGING_POD, pathFinder));
		trickyGrid[2][4].addActor(new Robot(blockedCellEight, 1, MockWarehouse.CHARGING_POD, pathFinder));
		trickyGrid[0][3].addActor(new Robot(blockedCellNine, 1, MockWarehouse.CHARGING_POD, pathFinder));
	}

	@Test
	public void testFindPathOnEmptyGridIgnoringObstacles() {
		BFS<Cell> BFS = new BFS<Cell>(EMPTY_GRID);
		Cell startLocation = EMPTY_GRID[5][2];
		Cell endLocation = EMPTY_GRID[0][4];

		PathLink<Cell> result = BFS.findPath(startLocation, endLocation, true);
	...
	@Test
	public void testFindPathOnEmptyGridNotIgnoringObstacles() {
		BFS<Cell> BFS = new BFS<Cell>(EMPTY_GRID);
		Cell startLocation = EMPTY_GRID[5][2];
		Cell endLocation = EMPTY_GRID[0][4];
	...
	@Test
	public void testFindPathWithCollisionsIgnoringObstacles() {
		BFS<Cell> BFS = new BFS<Cell>(trickyGrid);
		Cell startLocation = trickyGrid[5][2];
		Cell endLocation = trickyGrid[0][4];
